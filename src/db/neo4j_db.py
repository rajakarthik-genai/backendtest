...
from src.agents.llm_utils import llm_calculate_severity
...
class Neo4jDB:
    ...
    def calculate_severity_from_events(self, user_id: str, body_part: str) -> str:
        """Calculate severity level for a body part based on recent events using LLM."""
        if not self._initialized:
            raise RuntimeError("Neo4j not initialized")
        try:
            hashed_user_id = self._hash_user_id(user_id)
            with self.driver.session() as session:
                ...
                RETURN e.title as title, e.description as description, e.severity as severity, e.timestamp as timestamp
                ORDER BY e.timestamp DESC
                LIMIT 10
                """
                result = session.run(query, {
                ...
                if not events:
                    return "normal"
                # Use LLM for severity calculation
                try:
                    import asyncio
                    ...
                    if loop.is_running():
                        ...
                        import nest_asyncio
                        nest_asyncio.apply()
                        severity = loop.run_until_complete(llm_calculate_severity(body_part, events))
                    else:
                        severity = loop.run_until_complete(llm_calculate_severity(body_part, events))
                    if severity:
                        return severity
                ...
                severities = [event["severity"] for event in events if event["severity"]]
                severity_weights = {
                    "critical": 5,
                    "severe": 4,
                    "moderate": 3,
