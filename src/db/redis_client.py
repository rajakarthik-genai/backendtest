...
async def init_redis():
    ...
    _redis_client = await RedisClient.create()


async def get_redis_client():
    """Return the low-level Redis client with retry logic."""
    global _redis_client
    max_retries = 5
    retry_count = 0
    
    while _redis_client is None and retry_count < max_retries:
        try:
            await init_redis()
            if _redis_client is not None:
                return _redis_client
        except Exception as e:
            logger.warning(f"Redis retry {retry_count + 1}/{max_retries}: {e}")
