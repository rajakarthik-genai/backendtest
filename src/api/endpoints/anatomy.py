        ...
        if event_history:
            ...
            if recent_event:
                related_conditions = neo4j_client.get_related_conditions(
                    user_id, recent_event.get("title", "")
                )
...
async def update_body_part_severity(
    ...
    try:
        ...
        # Update severity
        success = neo4j_client.update_body_part_severity(user_id, body_part, severity)
...
async def auto_update_severities(current_user: CurrentUser):
    ...
    try:
        ...
        success = neo4j_client.auto_update_body_part_severities(user_id)
...
async def anatomy_overview(current_user: CurrentUser):
    ...
    try:
        ...
        hashed_user_id = neo4j_client._hash_user_id(user_id)
