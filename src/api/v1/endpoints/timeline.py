...
async def get_timeline_by_body_part(
    ...
    """
    Get timeline events for the authenticated user and a specific body part.
    """
    try:
        patient_id = current_user.patient_id
...
@router.get("/timeline/{year}", response_model=TimelineResponse)
async def get_timeline_by_year(
    current_user: CurrentUser,
    year: int = Path(..., description="Year to retrieve timeline for", ge=1900, le=2030),
    event_types: Optional[str] = Query(None, description="Comma-separated event types"),
    limit: int = Query(100, description="Maximum number of events")
):
    """
    Get chronological timeline of medical events for a specific year.
    
    This is a convenience endpoint that filters the timeline to a specific year.
    """
    try:
        patient_id = current_user.patient_id
        ...
        logger.info(f"Getting {year} timeline for patient {patient_id[:8]}...")
        
        # Get MongoDB timeline events
        mongo_client = await get_mongo()
        ...
        if event_types:
            ...
            all_events = [
                ...
                if event.get("type", "").lower() in [t.lower() for t in requested_types]
            ]
        ...
        log_user_action(
            patient_id,
            "timeline_year_retrieved",
            {
                "year": year,
                "event_count": len(all_events),
                "event_types_filter": event_types,
                "has_mongo_events": len(mongo_events) > 0,
                "has_neo4j_events": len(neo4j_events) > 0
            }
        )
        ...
        return TimelineResponse(
            ...
            timeline_period=f"Year {year}"
        )
        
    except Exception as e:
        logger.error(f"Failed to get timeline for year {year}: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to retrieve timeline for year {year}")


@router.get("/history/{year}", response_model=dict)
async def get_history_by_year(
    current_user: CurrentUser,
    year: int = Path(..., description="Year to retrieve history for", ge=1900, le=2030),
    include_summary: bool = Query(True, description="Include AI-generated summary"),
    include_statistics: bool = Query(True, description="Include event statistics")
):
    """
    Get comprehensive medical history for a specific year.
    
    This provides a more detailed view than the timeline endpoint, including:
    - Complete event timeline for the year
    - AI-generated summary of the year's medical events
    - Statistical breakdown of events by type, severity, and body parts
    """
    try:
        patient_id = current_user.patient_id
        
        # Get the timeline data for the year
        timeline_response = await get_timeline_by_year(current_user, year)
        ...
        if include_statistics:
            ...
            for event in events:
                ...
                # Count by body part
                body_part = event.get("body_part", "unknown")
                if body_part != "unknown":
                    stats["by_body_part"][body_part] = stats["by_body_part"].get(body_part, 0) + 1
                ...
                if timestamp:
                    try:
                        ...
                        if dt.year == year:  # Only count events from the requested year
        ...
        if include_summary and len(events) > 0:
            try:
                from src.agents.timeline_builder_agent import TimelineBuilderAgent
                
                timeline_agent = TimelineBuilderAgent()
                
                # Create a summary prompt for the year
                summary_prompt = f"""
                Please provide a comprehensive medical history summary for the year {year}.
                
                Total Events: {len(events)}
                
                Key Events:
                {chr(10).join([f"- {event.get('title', 'Untitled')}: {event.get('description', 'No description')}" for event in events[:10]])}
                
                Please provide:
                1. A narrative summary of the year's medical events
                2. Key health trends or patterns
                3. Notable changes in health status
                4. Recommendations based on the year's events
                """
                
                summary_result = await timeline_agent.generate_summary(
                    patient_id,
                    summary_prompt,
                    time_period=f"Year {year}"
                )
                
                if summary_result.get("success"):
                    history_data["ai_summary"] = {
                        "narrative": summary_result.get("summary", ""),
                ...
                logger.warning(f"Failed to generate AI summary for year {year}: {e}")
                history_data["ai_summary"] = {
        ...
        logger.error(f"Failed to get history for year {year}: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to retrieve history for year {year}")
