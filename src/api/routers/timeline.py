...
@router.post("/", response_model=List[TimelineEvent])
...
async def get_body_part_timeline(
    ...
    try:
        ...
        timeline_result = neo4j.execute_query(timeline_query, query_params)
        ...
        logger.error(f"Error in get_body_part_timeline: {e}")
...
async def get_patient_timeline(
    ...
    try:
        ...
        logger.error(f"Error in get_patient_timeline: {e}")
        raise HTTPException(status_code=503, detail="Database error")
...
async def get_timeline_summary(
    ...
    try:
        ...
        stats_result = neo4j.execute_query(stats_query, {"patient_id": patient_id})
        ...
        breakdown_result = neo4j.execute_query(breakdown_query, {"patient_id": patient_id})
        ...
        logger.error(f"Error in get_timeline_summary: {e}")
        raise HTTPException(status_code=503, detail="Database error")
...
async def get_timeline_events(
    ...
    """
    Get chronological timeline events for a patient
    ...
    try:
        ...
        if start_date:
            where_conditions.append("e.date >= $start_date")
            query_params["start_date"] = start_date.isoformat()
            
        if end_date:
            where_conditions.append("e.date <= $end_date")
            query_params["end_date"] = end_date.isoformat()
            
        if event_types:
            where_conditions.append("e.type IN $event_types")
            query_params["event_types"] = event_types
        
        where_clause = " AND ".join(where_conditions)
        
        # Get timeline events from Neo4j
        result = neo4j.execute_query(f"""
        ...
        for record in result:
            timeline_event = TimelineEvent(
                ...
                body_parts=record["body_parts"] or [],
                patient_id=patient_id
            )
            timeline_events.append(timeline_event)
        ...
        logger.error(f"Error in get_timeline_events: {e}")
        raise HTTPException(status_code=503, detail="Database error")
