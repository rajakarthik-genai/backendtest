...
async def get_streaming_expert_opinion(
    ...
    """
    try:
        # Get patient_id from request or extract from authenticated user
        ...
        # Database connection check with None comparison
        if db_clients.get("mongodb") is None or db_clients.get("neo4j") is None:
        ...
        logger.error(f"Error in get_streaming_expert_opinion: {e}")
...
async def get_quick_expert_opinion(
    ...
    try:
        ...
        logger.error(f"Error in get_quick_expert_opinion: {e}")
        raise HTTPException(status_code=503, detail="Database error")
