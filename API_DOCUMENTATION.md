...
## LLM-Driven & Dynamic Endpoints

Many endpoints (region mapping, severity calculation, timeline summaries, chat, expert opinion) are powered by advanced language models (OpenAI GPT). Prompts are versioned, hallucination-guarded, and centrally managed for best practices and traceability.

- **/anatomy/region/{region}**: Uses LLM to dynamically map regions to body parts and calculate aggregate severity.
- **/anatomy/body-part/{body_part}**: Uses LLM to assess current severity based on event history.
- **/timeline/summary**: Uses LLM to generate structured, narrative timeline summaries and insights.
- **/chat/message, /chat/stream, /expert_opinion/expert-opinion**: All chat and expert opinion endpoints are orchestrated by LLMs with multi-agent routing and aggregation.
- **Prompt Management**: All prompts are versioned, hallucination-guarded, and can be updated independently of code.
...
- **Patient Data Isolation**: All data is strictly isolated per patient using hashed IDs and unique constraints in MongoDB, Neo4j, Milvus, and Redis.
- **Admin Controls**: Admin-only endpoints for privileged operations (see /admin/* endpoints).
...
- **Chat Logs**: All chat messages (user and assistant) are persisted to MongoDB for long-term memory, in addition to Redis for short-term memory.
- **Profile**: User profile and long-term memory are persisted and updatable via /user/profile endpoints.
- **Document Deletion**: Full document deletion is supported across MongoDB, Neo4j, Milvus, and disk.
- **Prompt Management**: Prompts are centrally managed and versioned for all LLM-driven endpoints.
