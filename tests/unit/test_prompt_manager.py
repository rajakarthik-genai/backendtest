...
class TestPromptFileCompliance:
    def test_all_prompts_have_version_and_hallucination_guard(self):
        """Ensure all prompt files have 'version' and 'hallucination_guard' fields."""
        prompt_files = glob.glob(os.path.join(PROMPTS_DIR, '*.json'))
        assert prompt_files, "No prompt files found in src/prompts/"
        for prompt_file in prompt_files:
            with open(prompt_file, 'r') as f:
                data = json.load(f)
                assert 'version' in data, f"Missing 'version' in {prompt_file}"
                assert 'hallucination_guard' in data, f"Missing 'hallucination_guard' in {prompt_file}"
