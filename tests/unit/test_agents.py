...
class TestLLMDrivenRegionSeverity:
    @pytest.mark.asyncio
    async def test_llm_get_body_parts_for_region(self):
        """Test that llm_get_body_parts_for_region returns expected body parts from LLM."""
        region = "head"
        body_part_list = ["Head", "Brain", "Left Eye", "Right Eye"]
        # Mock the LLM call
        with patch('src.agents.llm_utils.AsyncOpenAI') as mock_openai:
            mock_client = AsyncMock()
            mock_openai.return_value = mock_client
            mock_client.chat.completions.create.return_value.choices = [
                MagicMock(message=MagicMock(content='["Head", "Brain"]'))
            ]
            result = await llm_get_body_parts_for_region(region, body_part_list)
            assert result == ["Head", "Brain"]

    @pytest.mark.asyncio
    async def test_llm_calculate_severity(self):
        """Test that llm_calculate_severity returns a valid severity from LLM."""
        body_part = "Heart"
        events = [
            {"title": "Chest pain", "severity": "severe", "timestamp": "2024-01-01T00:00:00"}
        ]
        with patch('src.agents.llm_utils.AsyncOpenAI') as mock_openai:
            mock_client = AsyncMock()
            mock_openai.return_value = mock_client
            mock_client.chat.completions.create.return_value.choices = [
                MagicMock(message=MagicMock(content='severe'))
            ]
            result = await llm_calculate_severity(body_part, events)
