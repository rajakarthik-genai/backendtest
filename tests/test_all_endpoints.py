...
class TestMediTwinAPI:
    ...
    def test_chat_message(self):
        """Test unified chat message endpoint with both standard and expert modes"""
        ...
        if response.status_code == 200:
            ...
            assert data["expert_mode"] == False
            assert "message_id" in data
            assert "timestamp" in data
        
        # Test expert opinion mode
        expert_payload = {
            "message": "What should I know about my heart condition?",
            "conversation_id": "test-session",
            "expert_opinion": True
        }
        response = requests.post(
        ...
        if response.status_code == 200:
            ...
            if data["comprehensive_analysis"]:
                assert "specialists_consulted" in data["comprehensive_analysis"]
                assert "confidence_score" in data["comprehensive_analysis"]
