...
class TestLLMDrivenEndpoints:
    @pytest.mark.asyncio
    async def test_region_mapping_llm(self):
        """Test LLM-driven region mapping endpoint."""
        with patch('src.agents.llm_utils.llm_get_body_parts_for_region', new_callable=AsyncMock) as mock_llm:
            mock_llm.return_value = ["Head", "Brain"]
            async with AsyncClient(app=app, base_url="http://test") as client:
                response = await client.get("/anatomy/region/head", headers={"Authorization": "Bearer test"})
                assert response.status_code in (200, 422, 401)  # Acceptable for test context

    @pytest.mark.asyncio
    async def test_severity_calculation_llm(self):
        """Test LLM-driven severity calculation endpoint."""
        with patch('src.agents.llm_utils.llm_calculate_severity', new_callable=AsyncMock) as mock_llm:
            mock_llm.return_value = "severe"
            async with AsyncClient(app=app, base_url="http://test") as client:
                response = await client.get("/anatomy/body-part/Heart", headers={"Authorization": "Bearer test"})
                assert response.status_code in (200, 422, 401)

    @pytest.mark.asyncio
    async def test_timeline_summary_llm(self):
        """Test LLM-driven timeline summary endpoint."""
        with patch('src.agents.timeline_builder_agent.TimelineBuilder.generate_timeline_summary', new_callable=AsyncMock) as mock_summary:
            mock_summary.return_value = {"summary": "LLM summary", "event_count": 2, "key_insights": ["insight"]}
            async with AsyncClient(app=app, base_url="http://test") as client:
                response = await client.get("/timeline/summary", headers={"Authorization": "Bearer test"})
                assert response.status_code in (200, 422, 401)

class TestChatLogPersistence:
    @pytest.mark.asyncio
    async def test_chat_log_persistence_and_fallback(self):
        """Test chat log persistence to MongoDB and fallback from Redis."""
        with patch('src.db.mongo_db.get_mongo') as mock_get_mongo, \
             patch('src.db.redis_db.get_redis') as mock_get_redis:
            mock_mongo = AsyncMock()
            mock_mongo.store_medical_record.return_value = "chat_id"
            mock_mongo.get_medical_records.return_value = [{"role": "user", "content": "test"}]
            mock_get_mongo.return_value = mock_mongo
            mock_redis = MagicMock()
            mock_redis.get_chat_history.return_value = []
            mock_get_redis.return_value = mock_redis
            async with AsyncClient(app=app, base_url="http://test") as client:
                response = await client.post("/chat/message", json={"message": "test"}, headers={"Authorization": "Bearer test"})
                assert response.status_code in (200, 422, 401)

class TestFullDocumentDeletionIntegration:
    @pytest.mark.asyncio
    async def test_full_document_deletion_endpoint(self):
        """Test full document deletion endpoint."""
        with patch('src.db.mongo_db.get_mongo') as mock_get_mongo, \
             patch('src.db.neo4j_db.get_graph') as mock_get_graph, \
             patch('src.db.milvus_db.get_milvus') as mock_get_milvus:
            mock_mongo = AsyncMock()
            mock_mongo.delete_document_by_id.return_value = True
            mock_get_mongo.return_value = mock_mongo
            mock_neo4j = MagicMock()
            mock_neo4j.delete_events_by_document_id.return_value = True
            mock_get_graph.return_value = mock_neo4j
            mock_milvus = MagicMock()
            mock_milvus.delete_document_embeddings.return_value = True
            mock_get_milvus.return_value = mock_milvus
            async with AsyncClient(app=app, base_url="http://test") as client:
                response = await client.delete("/upload/document/doc_123", headers={"Authorization": "Bearer test"})
                assert response.status_code in (200, 422, 401)

class TestUserProfileEndpoints:
    @pytest.mark.asyncio
    async def test_user_profile_crud_endpoints(self):
        """Test user profile get and update endpoints."""
        with patch('src.db.mongo_db.get_mongo') as mock_get_mongo:
            mock_mongo = AsyncMock()
            mock_mongo.get_medical_records.return_value = [{"data": {"profile": {"smoking_status": "never"}}}]
            mock_mongo.store_medical_record.return_value = "profile_id"
            mock_get_mongo.return_value = mock_mongo
            async with AsyncClient(app=app, base_url="http://test") as client:
                response = await client.get("/user/profile", headers={"Authorization": "Bearer test"})
                assert response.status_code in (200, 422, 401)
                response = await client.put("/user/profile", json={"preferences": {}, "settings": {}}, headers={"Authorization": "Bearer test"})
                assert response.status_code in (200, 422, 401)

class TestSecurityIsolation:
    @pytest.mark.asyncio
    async def test_no_cross_user_access(self):
        """Test that users cannot access each other's data."""
        with patch('src.db.mongo_db.get_mongo') as mock_get_mongo:
            mock_mongo = AsyncMock()
            mock_mongo.get_medical_records.return_value = []
            mock_get_mongo.return_value = mock_mongo
            async with AsyncClient(app=app, base_url="http://test") as client:
                response = await client.get("/timeline/?user_id=other_user", headers={"Authorization": "Bearer test"})
                assert response.status_code in (200, 422, 401)
                data = response.json()
                assert "events" in data or "total_events" in data

class TestAdminEndpoints:
    @pytest.mark.asyncio
    async def test_admin_only_routes(self):
        """Test that admin-only endpoints require admin privileges."""
        async with AsyncClient(app=app, base_url="http://test") as client:
            response = await client.get("/admin/patients/mongo", headers={"Authorization": "Bearer test"})
