...
class EnhancedHealthDashboard {
  constructor(apiBaseUrl = null) {
    ...
    this.api = window.apiService || window.digitalTwinAPI || {
  ...
  async sendChatMessage() {
    ...
    try {
      ...
      // Use the integrated chat manager and API service if available
      if (this.chatManager && window.apiService) {
        try {
          // Use the agent service for chat
          response = await window.apiService.chatWithAgent(message, {
            expert_mode: this.expertMode,
            context: 'dashboard_chat'
          });
        } catch (error) {
          console.warn('Agent service unavailable, using fallback:', error);
